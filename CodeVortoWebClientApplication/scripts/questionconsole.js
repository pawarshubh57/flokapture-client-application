    ...
    */
    if (typeof srcResults !== "undefined" && srcResults !== null && srcResults !== "null") {
        ...
        if (searchResults !== null) {
            ...
            /*
            var stmtComments = searchOpt.stmtComments;
        
            if (rdExcludeComt === true) {
                objectStatements =  false;
                document.getElementById("chkObjectFilesStatements").checked = false;
                $("#chkObjectFilesStatements").closest("label").removeClass("active");
                $("#chkObjectFilesStatements").attr('disabled', 'disabled');
            }
            */
            /*
            $("#dvObjectStatement")[0].style.visibility = "hidden";
            $("#dvObjectStatement")[0].style.display = "none";
            */
            /*
            if (rdExcludeComt) {
            ...
            if (rdPreText) {
                ...
                $("#chkObjectFilesStatements").removeAttr('disabled', 'disabled');
            }
            */
            if (rdExcludeComt) {
                document.getElementById("rdExcludeComt").checked = true;
            ...
            /*
            if (!stmtComments) {
                document.getElementById("chkStmtComment").checked = false;
            } else {
                document.getElementById("chkStmtComment").checked = true;
                if (rdExcludeComt) {
                    $("#dvStatementComments")[0].style.visibility = "visible";
                    $("#dvStatementComments")[0].style.display = "inline";
                    fillStatmentCommentsData(resultStmtComment);
                }
                if (rdPreText) {
                    $("#dvStatementComments")[0].style.display = "none";
                    $("#dvStatementComments")[0].style.visibility = "hidden";
                }
            }
          */
...
$(document).ready(function () {
    ...
    $("#btnSearch").click(function () {
        ...
        /* Discuss it later boz before it is suggest 
        if (rdExcludeComt === true) {
            objectStatements = false;
            document.getElementById("chkObjectFilesStatements").checked = false;
            $("#chkObjectFilesStatements").closest("label").removeClass("active");
            ...
            jQuery.ajax({
                ...
                success: function (searchResults) {
                    ...
                    if (typeof searchResults !== "undefined" && searchResults !== null) {
                        ...
                        // resultStmtComment = searchResults[5].ListWorkflowTreeviewSecondTabDetails;
                        resultWf = resultWf || [];
                        if (typeof resultWf === "undefined" || resultWf === null)
                            resultWf = [];
                        if (typeof resultTn === "undefined" || resultTn === null)
                        ...
                        keyworkSearchResults = {
                            ...
                            // resultStmtComment: resultStmtComment,
                    ...
                    if (typeof resultWf !== "undefined") {
                        $("#tblSearchResult").ejGrid({
                            ...
                            columns: [
                                ...
                                // { field: "WorkflowBusinessName", headerText: "Workflow Name Description", width: "31%" },
                ...
                if (opt === 0) {
                    ...
                    if (typeof extensionIds !== "undefined" && extensionIds !== null && extensionIds !== "") {
                        ...
                        $.each(ps, function (i, k) {
                            ...
                            var item = $("#ddlObjectType").jqxDropDownList("getItemByValue", v);
                            if (typeof item !== "undefined" && item !== null) {
                                // if (item.originalItem.ProjectId === parseInt(prjctId)) {
                                $("#ddlObjectType").jqxDropDownList("checkItem", item);
                                // }
                            }
                        });
...
function fillWorkflowSearchData(resultWf) {
    //  $("#tblSearchKeyWords").ejGrid("setWidthToColumns");
    if (typeof resultWf !== "undefined") {
        var l = 1;
        // ReSharper disable once QualifiedExpressionMaybeNull
        // - (Total Count: &nbsp;55)
    ...
    if (typeof resultStmtComment !== "undefined" && resultStmtComment !== null) {
        ...
        $("#tblStmtComments").ejGrid({
            ...
            columns: [
                ...
                //{ field: "RowId", headerText: "Sr#", width: 50 },
                //{ field: "View", headerText: "Statement Comment", width: 200 },
                //{ field: "StatementView", headerText: "Custom View", width: 200 },
                //{ field: "ClassCalled", headerText: "Object Name", width: 250 },
                //{ field: "ObjectType", headerText: "Object Type", width: 150 },
                //{ field: "WorkflowBusinessName", headerText: "Match Type" }
            ],
            queryCellInfo: function (args) {
                $(args.cell).attr({
                    "data-toggle": "tooltip",
...
function doChange(args) {
    ...
    //if (obj.currentValue !== "Select All Items") {
    //    $('#ddlSelectProject').ejDropDownList("unselectItemsByIndices", "0");
    //} else {
...
function openLink(prjId, stId, aId) {
    ...
    var url = pageLink; // thisDiv.innerHTML.replace(/&amp;/g, '&');
    if (parentWin) {
...
$("input[type=radio]").change(function () {
    /*
    var rdExcludeComt = $('#rdExcludeComt:checked').val() ? true : false; 
    if (rdExcludeComt === true) {
        document.getElementById("chkObjectFilesStatements").checked = false;
        $("#chkObjectFilesStatements").closest("label").removeClass("active");       
        $("#chkObjectFilesStatements").attr('disabled', 'disabled');
        ...
        document.getElementById("chkObjectFilesStatements").checked = true;
        $("#chkObjectFilesStatements").removeAttr('disabled', 'disabled');
        $("#chkObjectFilesStatements").closest("label").addClass("active");
    }
    */
})