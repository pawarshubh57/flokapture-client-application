...
$(document).ready(function () {
    ...
    $("#btnSearchText").click(function () {
        ...
        if (hitHighlighter.hitCount === 0) {
            ...
            $("#divSearchNav").show();
            $("#divSearchHitCount").text(hitHighlighter.hitCount + (hitHighlighter.hitCount === 1 ? " match" : " matches"));
        }
    });
    */
    $("#btnSearchText").click(function () {
    ...
    $("#txtSearchBox").keypress(function (e) {
        if (e.keyCode === 13) {
            hitHighlighter.apply($("#txtSearchBox").val());
            if (hitHighlighter.hitCount === 0) {
                $("#divSearchHitCount").text("no matches found");
            } else {
                $("#divSearchNav").show();
    ...
    */
    ...
    /*
    $("#btnSearchTextNew").click(function () {
        var searchText = $("#txtSearchBoxNew").val();
        hitHighlighterNew.apply(searchText);
        if (hitHighlighterNew.hitCount === 0) {
            $("#divSearchHitCountNew").text("no matches found");
        } else {
            $("#divSearchNavNew").show();
            $("#divSearchHitCountNew").text(hitHighlighterNew.hitCount + (hitHighlighterNew.hitCount === 1 ? " match" : " matches"));
        }
    });
    */
    /*
    $("#txtSearchBoxNew").keypress(function (e) {
        if (e.keyCode === 13) {
            hitHighlighterNew.apply($("#txtSearchBoxNew").val());
            if (hitHighlighterNew.hitCount === 0) {
                $("#divSearchHitCountNew").text("no matches found");
            } else {
                $("#divSearchNavNew").show();
...
function saveObjectDocuments(path, fileName) {
    ...
    jQuery.ajax({
        ...
        data: objectDoc, // fileUploadDetails[0],
        success: function (result) {
            if (result !== null) {
                getCurrentAttachement(programId);
            }
...
function getCurrentAttachement(programId) {
    ...
    objectDocument.getAttachementObjects(programId, projectId).then(function (d) {
        d.forEach(function (object) {
            ...
            /*
            var isImg = /\.(jpe?g|png|gif|bmp)$/i.test(downloadLink);
            var imgEle = $("<img />").attr({
                "id": "imgZoom",
                "width": "60px",
                "height": "60px",
            ...
            imgEle.on("mousemove", { iLink: downloadLink }, zoomIn).on("mouseout", { iLink: downloadLink }, zoomOut);
            */
            /*
            var overlay = $("<div />").attr({ "id": "overlay" }).css({
                "border": "1px solid black", "width": "550px", "height": "400px", "display": "inline-block",
                "background-image": "url(" + downloadLink + ")",
                "background-repeat": "no-repeat"
            });
            */
            // headerRow.append($("<td />").append(object.FileName));
            ...
            // headerRow.append($("<td />").append(isImg ? imgEle : $("")));
...
function showEntityDetails() {
    ...
    jQuery.ajax({
        ...
        success: function (entryPoints) {
            ...
            var gridObjDescriptor = $("#divUniverseDecriptor").data("ejGrid");
            gridObjDescriptor.refreshContent(); // Refreshes the grid contents only
            gridObjDescriptor.refreshContent(true); // Refreshes the template and grid contents
            */
        }
    });
}
...
function callPseudoView() {
    ...
    // var fileId = window.localStorage.getItem("fileId");
    jQuery.ajax({
        type: "GET",
...
$(document).ready(function () {
    $("#btnAddTag").click(function () {
        ...
        // Check here whether Tags are frozen for current user logged in..
        // If yes, then add Tag history in audit history table, else insert tag details without auditing it...
        /*
        var isFrozen = true;
        if (isFrozen) {
            var audit = {
                postData: {
                    OptionUsed: "Associated Items",
                    PrimaryScreen: "Freeze Tags",
                    UserId: userId,
                    ProjectId: pid,
                    BriefDescription: "Request for freezing tags is initiated."
                }
            };
            $.fn.auditActionLog(audit).then(function () {
                return false;
