...
function bindData() {
    ...
    var name = window.localStorage.getItem("connectivityName");
    if (name === "Project Connectivity with Dictionary") {
        connectivityViaDictionary();
    } else if (name === "Project Connectivity" || name === "Overall Connectivity") {
        loadObjectConnectivity();
    } else if (name === "Project Portfolio Workflow") {
        loadData(projectId, 0, 2, 0);
    } else if (name === "Overall Workflow") {
        loadData(0, 0, 2, 0);
    } else if (name === "Multiple projects") {
        var workFlowId = window.localStorage.getItem("workFlowId");
        loadData(projectId, 0, 1, workFlowId);
    }
}

function loadData(projectId, stmtId, param, workflowId) {

    var name = window.localStorage.getItem("connectivityName");
    //  $.get(baseAddress + "PostProcess/GetAlternateWorkflow?projectId=" + projectId + "&stmtId=" + stmtId,
    $.get(baseAddress + "FileObjectReference/GetGroupedFlowChart?workFlowId=" + workflowId + "&projectId=" + projectId + "&stmtId=" + stmtId + "&param=" + param + "&name=" + name + "",
        function(tData) {
...
function buildDiagram(lstNodes, lstLinks) {
    diagram = $create(Diagram, null, null, null, $get("canvasObjectConnectivity")); // Diagram.find("diagram");
    ...
    Array.forEach(nodes, function(node) {
        ...
        //diagramNode.njSoftId = node.NjSoftId;
    });
...
function connectivityViaDictionary() {
    ...
    jQuery.ajax({
        ...
        type: 'GET',
        data: { "projectId": projectId, "name": name, "param": "dictionaryConnectivity" },
        contentType: "application/json;charset=utf-8",
        success: function(data) {
            if (data.Nodes) {
...
function loadObjectConnectivity() {
    ...
    jQuery.ajax({
        url: baseAddress + "FileObjectReference/GetObjectReference",
        type: 'GET',
        data: { "projectId": projectId, "name": name, "param": "connectivity" },
        contentType: "application/json;charset=utf-8",
        success: function(data) {
            if (data != null) {
                MainData = data;
