...
function fillUserDetailsGrid() {
    jQuery.ajax({
        type: "GET",
        url: baseAddress + "UserMaster/GetAllItems",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            for (var i = 0; i < dataDetails.length; i++) {
                userDetailsNew.push({
                    ...
                    "Status": data[i].UserDetails[0].Status,
                    "FirstName": data[i].UserDetails[0].FirstName,
                    "LastName": data[i].UserDetails[0].LastName,
                    "Role": data[i].UserDetails[0].RoleMaster.RoleType,
                    "UserId": data[i].UserId,
                    "UserDetailsId": data[i].UserDetails[0].UserDetailsId
                });
            }
            if (data.length > 0) {
                $("#userDetails").ejGrid(
...
function editUserDetails() {
    ...
    if (currentUserId !== "") {
        ...
        $("#divAdd").hide();
        $("#divUpdate").show();
        $.get(baseAddress + "UserMaster/GetItems?tKey=" + currentUserId, function (result) {
            if (result !== null) {
                //$("#ddlRoles12 option").remove();
                //document.getElementById("#ddlRoles12").innerHTML = "";
                var userDetails = result.UserDetails[0];
        ...
        success: function (result) {
            ...
            if (result != null) {
                $.each(result, function (key, value) {
                    $("#ddlRole").append("<option value=" + value.Value + ">" + value.Name + "</option>");
                    $("#ddlRoles12").append("<option value=" + value.Value + ">" + value.Name + "</option>");
                });
            }
        }
    });
...
function fillProjectDropDownUpdate() {
    ...
    jQuery.ajax({
        ...
        success: function (result) {
            // $('#ddlProjectNameUpdate').ejDropDownList("clear");
            if (result != null) {
                $.each(result, function (key, value) {
    ...
    jQuery.ajax({
        ...
        success: function (result) {
            if (result != null) {
                $.each(result, function (key, value) {
                    if (value.Value === 0) return;
                    $("#ddlProjectName").append("<option value=" + value.Value + ">" + value.Name + "</option>");
                });
